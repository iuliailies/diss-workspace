...
export class IndexComponent implements OnInit{
    ...
    constructor(private noteService: NoteService, private dialogBox:MatDialog, private notificationService: NotificationService) {
    }
      ...
      dialogResponse.afterClosed().subscribe( response => {
        if(response){
          this.noteService.deleteDocument(document.id).subscribe({
            next: () => {
              this.fetchDocuments();
              this.notificationService.notify({message: 'Document deleted successfully!', type: NotificationType.error});
            },
            error: (error: any) => {
              if (error.error instanceof ErrorEvent) {
                this.notificationService.notify({message: 'An error occurred! Please try again later!', type: NotificationType.error});
              } else {
                const errResponse: ErrorResponseModel = error.error as ErrorResponseModel;
                this.notificationService.notify({message: errResponse.errorMessage, type: NotificationType.error})
              }
            }
          })
        }
