...
export class CommentsComponent implements OnInit {
  ...
  deleteComment(comment: Comment, index: number): void {
    const dialogResponse = this.confirmationDialogService.confirm(`Are you sure you want to delete the comment?`);
  ...
  isCommentEditable(comment: Comment) : boolean {
    return comment.user.id === parseInt(this.userId || '-1') && this.commentIndexEdit === -1
  }
